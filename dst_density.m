...
for g=1:length(L)
    ...
    val=zeros(1,length(d));
       ...
       plot(new*1000000000,(16*final_d)/(1000000),'linewidth',1.5)
       hold on
end
...
plot(new.*1000000000,base,'--','linewidth',1.5) 
grid on
box on
ax = gca; % current axes
ax.FontSize = 14;
...
ylabel('$$\mathrm{Methane~density,~\frac{gm}{cm^3}}$$','interpreter','latex')
s = ...
    {
     '$$0.4~\mathrm{nm}$$'
     '$$0.5~\mathrm{nm}$$';
...
print('-depsc2','-r400','dst_density.eps');